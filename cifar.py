...
def prepare_dataloader(num_workers=8,
    ...
    # --------------- CIFAR10 : Train + Quant ---------------
    # FIXME: mean and std of ImageNet val
    """
    FIXME: why use ImageNet's mean & std
    ...
    # FIXME: which is SequentialSampler
    # test_sampler = SequentialSampler( test_set )

    N_calibration = 2000
    indices = random.sample( range(len(test_loader)), N_calibration )
    calibration_subset = Subset( test_loader, indices )
    calibration_loader = DataLoader( datase=test_set,
                                     batch_size=32,
                                     shuffle=False,
                                     num_workers=num_workers,
                                     pin_memory=False )
...
def create_model(pretrained=False, **kwargs):
    ...
    if pretrained:
        ...
        """
        FIXME: 传递num_classes请参考hyperai中的lsq-net项目
        """
...
def main():
    ...
    """
    FIXME:
      why load saved_model instead of using trained_model ??
    """
